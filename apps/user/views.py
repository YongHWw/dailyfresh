...
#         user.save()
#         或者
#         User.object.create(username=username, password=password, email=email)
#         '''
...
class register(View):
    ...
    def post(self, request):
        ...
        # 返送激活邮件，包含激活链接 ：http://127.0.0.1:8000/user/activate/3
        # 激活链接中需要包含用户的身份信息, 并且把身份信息进行加密(itsdangerous模块)
        ...
        # html_message = '<h1>%s, 欢迎您成为天天生鲜注册会员</h1>请点击下面链接激活您的账户<br/><a href="http://127.0.0.1:8000/usr/active/%s">http://127.0.0.1:8000/user/active/%s</a>'%(username, token, token)
        #
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        # page='user'
        # Django会给request对象添加一个 属性request.user
        ...
        context = {'page': 'user',
                   'address': address,
                   'goods_li': goods_li}
        # 除了你给模板文件传递的模板变量之外，django本身会直接把request.user也传递给模板文件
